...
for idx, rectangle in enumerate(rectangles):
    rectangle_rect = fabric[rectangle.left:rectangle.right, rectangle.top:rectangle.bottom]
    rectangle_ones = rectangle_rect[rectangle_rect != 1]
    if len(rectangle_ones) == 0:
        print(idx + 1, 'execution time: {:.2f} seconds'.format(time.time() - start))
